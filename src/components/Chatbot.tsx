...
function formatPrice(n: number) { return n.toLocaleString('vi-VN') + 'd'; }
  ...
  return (
    <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden mt-2">
      ...
      <div className="p-3 space-y-3">
        ...
        <div>
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">⏰ Khung gio</label>
        ...
        {date && time && (
          <div className="bg-slate-50 rounded-xl px-3 py-2 space-y-1 border border-slate-100">
            ...
            <div className="flex justify-between text-xs text-slate-600">
              ...
              <span className="font-semibold">{payMethod === 'CASH' ? 'Tien mat tai quay' : 'PayOS (QR)'}</span>
            </div>
            <div className="flex justify-between text-xs border-t border-slate-200 pt-1 mt-1">
              <span className="font-bold text-slate-800">Tong cong</span>
        ...
        <button onClick={handleConfirm} disabled={!canConfirm}
          ...
          {confirming
            ? <><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Dang xu ly...</>
            : payMethod === 'PAYOS' ? '📱 Thanh toan qua PayOS' : '✅ Xac nhan dat lich (Tien mat)'}
        </button>
      </div>
    </div>
  );
...
function ToolResultRenderer({ result, onBookingConfirm }: {
  ...
  onBookingConfirm?: (datetime: string, payMethod: PayMethod, data: BookingPickerData) => void;
}) {
  ...
  if (result.type === 'booking_picker') {
    ...
    return <BookingPickerCard data={d} onConfirm={(dt, pm) => onBookingConfirm?.(dt, pm, d)} />;
  }
...
const QUICK_SUGGESTIONS = [
  { icon: '⭐', text: 'Shop danh gia cao nhat' },
  { icon: '✂️', text: 'Tim dich vu grooming' },
  { icon: '🐾', text: 'Thu cung cua toi' },
  { icon: '🏨', text: 'Dich vu luu tru' },
  { icon: '💉', text: 'Tiem phong cho thu cung' },
  { icon: '🏥', text: 'Phong kham thu y' },
];

// ── Main Chatbot Component ───────────────────────────────────────────────────
const WELCOME_MSG_FN = (name?: string) =>
  'Xin chao' + (name ? ' **' + name + '**' : '') + '! 👋 Toi la **PetEye Assistant**.\n\nToi co the giup ban:\n• Tim shop cham soc thu cung phu hop\n• Xem thong tin thu cung cua ban\n• Goi y dich vu theo nhu cau\n• Dat lich hen truc tiep\n\nBan can tim gi hom nay?';
...
export default function Chatbot() {
  ...
  }, [input, isProcessing, open, sendMessage]);

  // Booking confirm — still handled on FE since it needs PayOS redirect
  const handleBookingConfirm = useCallback(async (datetime: string, payMethod: PayMethod, pickerData: BookingPickerData) => {
    setLoading(true);
    try {
      if (payMethod === 'PAYOS') {
        ...
        if (result.checkoutUrl) {
          await sendMessage('Dang chuyen den trang thanh toan PayOS cho ' + pickerData.petName + '...');
  ...
  const handleClearHistory = useCallback(async () => {
    if (!window.confirm('Xoa toan bo lich su chat?')) return;
    await clearHistory();
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-[#1a2b4c] to-indigo-600 shrink-0">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-indigo-200 text-xs">Luon san sang ho tro</span>
              </div>
            </div>
          ...
          {showHistory ? (
            <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-white">
                <p className="font-bold text-slate-800 text-sm">Lich su ({realMessages.length} tin)</p>
                <button onClick={handleClearHistory} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 font-semibold">
                  <span className="material-symbols-outlined text-sm">delete</span>Xoa tat ca
                </button>
              </div>
              {realMessages.length === 0
                ? <div className="flex-1 flex items-center justify-center text-slate-400 text-sm">Chua co lich su chat</div>
              ...
              <div className="p-3 border-t border-slate-200 bg-white">
                <button onClick={() => setShowHistory(false)} className="w-full py-2 bg-[#1a2b4c] text-white text-xs font-bold rounded-xl hover:bg-[#243d6b] transition-colors">Quay lai chat</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="px-3 py-3 bg-white border-t border-slate-100 shrink-0">
                {!user && (
                  <div className="mb-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700 flex items-center gap-2">
                    ...
                    <span><Link to="/login" className="font-bold underline">Dang nhap</Link> de luu lich su & dat lich</span>
                  </div>
                )}
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                  <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                    placeholder="Nhap cau hoi..." disabled={isProcessing}

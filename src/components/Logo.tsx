...
export default function Logo({ className = '', lightText = false }: { className?: string, lightText?: boolean }) {
    return (
        <div className={`flex items-center gap-3 group ${className}`}>
            ...
            <span className={`text-xl md:text-2xl font-black tracking-tight ${lightText ? 'bg-gradient-to-r from-white to-secondary bg-clip-text text-transparent' : 'bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent dark:from-white dark:to-secondary'}`}>
                PetEye
            </span>
        </div>
    );

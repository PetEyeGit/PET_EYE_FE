...
function getShopStatus(s: AdminShopResponse): ShopStatus {
  ...
  // fallback nếu BE chưa trả status
...
function StatusBadge({ status }: { status: ShopStatus }) {
  ...
  if (status === 'REJECTED') return (
    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-red-100 text-red-700">
      <Ban size={11} /> Từ chối
    </span>
  );
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-yellow-100 text-yellow-700">

...
// ─── Quick actions ────────────────────────────────────────────────────────────
const QUICK_ACTIONS = [
  { icon: '📊', label: 'Tổng quan hệ thống', color: 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100', prompt: 'Phân tích tổng quan hệ thống PetEye: doanh thu, người dùng, shop, booking. Nhận xét xu hướng và đề xuất cải thiện.' },
  { icon: '🏪', label: 'Phân tích Shop', color: 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100', prompt: 'Phân tích tình trạng các shop: tổng số, đã duyệt, chờ duyệt, shop nào cần chú ý. Đề xuất chính sách quản lý shop.' },
  { icon: '⏳', label: 'Shop chờ duyệt', color: 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100', prompt: 'Liệt kê và phân tích các shop đang chờ duyệt. Tiêu chí nào cần kiểm tra? Đề xuất quy trình duyệt nhanh hơn.' },
  { icon: '👥', label: 'Phân tích Member', color: 'bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100', prompt: 'Phân tích người dùng: tổng số, phân loại theo role, tăng trưởng. Đề xuất chiến lược giữ chân và thu hút user mới.' },
  { icon: '💬', label: 'Tình trạng tin nhắn', color: 'bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100', prompt: 'Phân tích tin nhắn chưa đọc và tình trạng hỗ trợ khách hàng. Đề xuất cải thiện thời gian phản hồi.' },
  { icon: '⚠️', label: 'Rủi ro & cảnh báo', color: 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100', prompt: 'Có rủi ro gì trong hệ thống? Shop vi phạm, user bất thường, doanh thu giảm, tin nhắn tồn đọng? Đề xuất xử lý ngay.' },
  { icon: '📋', label: 'Chính sách đề xuất', color: 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100', prompt: 'Dựa trên dữ liệu thực tế, đề xuất các chính sách mới cho nền tảng PetEye: chính sách shop, user, thanh toán, chất lượng dịch vụ.' },
  { icon: '📈', label: 'Chiến lược tăng trưởng', color: 'bg-cyan-50 text-cyan-700 border-cyan-200 hover:bg-cyan-100', prompt: 'Đề xuất 3-5 chiến lược cụ thể để tăng trưởng nền tảng PetEye trong quý tới: thu hút shop mới, tăng user, tăng booking.' },
];
...
export default function AdminAIAssistant() {
  ...
  // Stats for display (still fetched on FE for the stat bar)
    ...
    <div className="flex flex-col h-full bg-[#f8fafc]">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { refetchStats(); refetchShops(); }} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 hover:text-blue-600 transition-colors" title="Làm mới dữ liệu">
            <RefreshCw size={16} />
          </button>
          <button onClick={clearHistory} className="p-2 rounded-xl hover:bg-red-50 text-slate-500 hover:text-red-500 transition-colors" title="Xóa lịch sử">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="px-6 pt-3 pb-2 shrink-0 bg-white border-b border-slate-100">
        <button onClick={() => setShowQuickActions(v => !v)} className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 mb-2 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.map(msg => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-white border border-slate-100 rounded-tl-sm'}`}>
              {msg.isLoading ? (
      ...
      <div className="px-6 py-4 bg-white border-t border-slate-100 shrink-0">
        ...
        <p className="text-[10px] text-slate-400 mt-2 text-center">Enter để gửi · Shift+Enter xuống dòng</p>
      </div>
    </div>
  );
}
